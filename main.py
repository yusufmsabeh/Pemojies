...
#   print(db[n])
  ...
  # with open('DataBaseOpenar', 'a') as f:
...
async def on_ready():
    ...
    activity = discord.Game(name="phelp")
...
async def on_message(message):
    ...
    if message.guild.name=="FF 15":
      ThereAtta=False;
      # lenAttachments = len(message.Attahcments)
      with open('DataBaseOpenar','a') as f:
        
        for attacment in message.attachments:
          ThereAtta=True
          f.write(f" \n {message.channel}\n {str(message.created_at)[0:-7]}\n {message.author}\n {message.content}\n { attacment.url} \n\n----------------\n\n");
      if not ThereAtta:
         with open('DataBaseOpenar','a') as f:
          f.write(f" \n  {message.channel}\n  {str(message.created_at)[0:-7]}\n  {message.author}\n  {message.content} \n\n----------------\n\n");
          
       
    # if str(message.author)=='! SaBouBa#0006' :
    ...
    if message.author == client.user or len(message.content)==0:
        ...
        # Printing help message
            ...
            embed.add_field(name="premove",value="To remove you from the database",inline=True)
    ...
    # ****************padd method****************
            ...
            if not (user_id in db.keys()):
              ...
              await message.channel.send(embed=embed)
                ...
                index_of_com=lower_message.find("=")
                if '=' in lower_message[:index_of_com+1] and "=" in lower_message[index_of_com+1:]:
                  ...
                  embed.set_author(name=message.author.display_name , icon_url=message.author.avatar_url)
                  embed.set_footer(text="Type it with =")
                  embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/821673459716587551/826021805911179284/attention.png")
                  await message.channel.send(embed=embed)
...
#**********************************Pdel fun************************************************
        ...
        #*****************************pinvte*************************
          ...
          user_men = await client.fetch_user (654295662191443968)
          await user_men.send("see u after 3 monthes asshole")
        ...
        elif lower_message.startswith('premove'):
          if not (user_id in db.keys()):
              ...
              del db[user_id]
              del db[f"in{user_id}"]
              embed = discord.Embed(title=" removed successfully",description="Your data removed",color=0xFF5733)
              embed.set_author(name=message.author.display_name , icon_url=message.author.avatar_url)
              embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/674654477751418914/852481740235997224/remove.png")
              await message.channel.send(embed=embed)
        ...
        #************************how many***************************
          ...
          if str(message.author.id)==os.getenv('id'):
            ...
            print(str(message.author.id)+" knows phmy command")
        #******************pvote command****************************

        elif lower_message.startswith('pvote'):
          embed=discord.Embed(title="Voting",description="[:slight_smile:  vote link  ](https://top.gg/bot/821037249108901918/vote)")
          embed.set_footer(text="Thanks for helping us")
